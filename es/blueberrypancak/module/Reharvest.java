...
public class Reharvest extends Module {
	...
	public void onTick(EventTick e) {
		if (isEnabled() && (InventoryHelper.isItem(lastSlot, BONEMEAL) || equipBonemeal())) {
			...
			if (System.currentTimeMillis() >= nextTick && (p.noClip || p.getDistanceSq(soil) < 6)) {
				String seeds = mc.world.getBlockState(soil.up()).toString();
				String block = mc.world.getBlockState(soil).toString();
				if (seeds.contains("wheat")) {
					if (!seeds.contains("age=7")) {
						p.getConnection().sendPacket(new CPacketPlayerTryUseItemOnBlock(soil.up(), EnumFacing.UP,
								EnumHand.MAIN_HAND, 0, 0, 0));
						p.getConnection().sendPacket(new CPacketAnimation(EnumHand.MAIN_HAND));
					} else {
						p.getConnection().sendPacket(new CPacketPlayerDigging(
								CPacketPlayerDigging.Action.START_DESTROY_BLOCK, soil.up(), EnumFacing.DOWN));
						p.getConnection().sendPacket(new CPacketPlayerDigging(
								CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, soil.up(), EnumFacing.DOWN));
						p.getConnection().sendPacket(new CPacketAnimation(EnumHand.MAIN_HAND));
					}
				} else if (block.contains("moisture")) {
					p.getConnection().sendPacket(new CPacketPlayerTryUseItemOnBlock(soil, EnumFacing.UP,
							EnumHand.OFF_HAND, soil.getX(), soil.getY(), soil.getZ()));
	...
	public boolean isEnabled() {
		return super.isEnabled()
				&& Item.getIdFromItem(Client.getMinecraft().player.getHeldItemOffhand().getItem()) == WHEAT_SEEDS
				&& findSoil();
	}
	...
	private boolean findSoil() {
		...
		if (soil == null && mc.objectMouseOver != null && mc.objectMouseOver.typeOfHit == RayTraceResult.Type.BLOCK) {
			...
			if (!block.contains("moisture")) {
				soil = soil.down();
				block = mc.world.getBlockState(soil).toString();
				if (!block.contains("moisture")) {
					soil = null;
				}
			}
