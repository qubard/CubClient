...
public class Waypoint extends Module {
	...
	public void onChat(EventChat e) {
		...
		if(message.startsWith("-f")) {
			if(message.split(" ").length == 4) {
				...
				this.marker = new Marker(x,(int)Client.getMinecraft().thePlayer.posY,z);
			} else {
				this.marker = null;
			}
			e.setCancelled(true);
	...
	private void renderMarker(Marker m) {
		...
		GL11.glRotatef(Client.getMinecraft().gameSettings.thirdPersonView == 2 ? -r.playerViewX : r.playerViewX, 1.0F, 0.0F, 0.0F);
